...
object Instant {
  ...
  def now(clock: Clock): Instant = java.time.Instant.now(clock)
...
object Duration {
  ...
  def apply(duration: SDuration): Duration = Duration(duration.toMillis, ChronoUnit.MILLIS)
  ...
  def parse(duration: String): Duration = apply(SDuration(duration))
...
object LocalDate {
  ...
  def apply(epochDay: Long): LocalDate = java.time.LocalDate.ofEpochDay(epochDay)
}
...
object ZonedDateTime {
  ...
  def apply(instant: Instant, zone: ZoneId): ZonedDateTime =
...
object OffsetDateTime {
  ...
  /** @param iso8601DateTime A date on the format '2007-12-03T10:15:30+01:00', see
   *                        [[java.time.ZonedDateTime.parse()]]
   * @see java.time.OffsetDateTime.parse(String)
   */
  def apply(iso8601DateTime: String): OffsetDateTime =
    java.time.OffsetDateTime.parse(iso8601DateTime)
}
...
object YearMonth {
  ...
  def apply(yearMonth: String, formatter: DateTimeFormatter): YearMonth =
    java.time.YearMonth.parse(yearMonth, formatter)
...
object DateTimeFormatter {
  ...
  final val IsoLocalDateTime: DateTimeFormatter =
    java.time.format.DateTimeFormatter.ISO_LOCAL_DATE_TIME

  final val IsoOffsetDateTime: DateTimeFormatter =
    java.time.format.DateTimeFormatter.ISO_OFFSET_DATE_TIME

  final val IsoZonedDateTime: DateTimeFormatter =
  ...
  final val Rfc1123DateTime: DateTimeFormatter =
  ...
  def apply(
             dateStyle: Option[FormatStyle],
             timeStyle: Option[FormatStyle]): Option[DateTimeFormatter] = {
    (dateStyle, timeStyle) match {
      case (Some(dateStyleV), None) =>
        Some(java.time.format.DateTimeFormatter.ofLocalizedDate(dateStyleV))
      case (None, Some(timeStyleV)) =>
        Some(java.time.format.DateTimeFormatter.ofLocalizedTime(timeStyleV))
